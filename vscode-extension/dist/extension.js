...
var CodelensProvider = class {
  constructor() {
    ...
    this.decisionData = /* @__PURE__ */ new Map();
  ...
  provideCodeLenses(document, token) {
    ...
    if (relatedDecisions) {
      ...
      lenses.push(new vscode.CodeLens(range, {
        title: `\u{1F4DA} Decisions: ${titles}`,
...
var WebViewPanel = class _WebViewPanel {
  ...
  static createOrShow(extensionUri) {
    ...
    if (_WebViewPanel.currentPanel) {
      _WebViewPanel.currentPanel._panel.reveal(column);
      return;
    }
    const panel = vscode2.window.createWebviewPanel(
      "decisionTapestryDashboard",
      // Identifies the type of the webview. Used internally
      "Decision Tapestry Dashboard",
  ...
  focusOnDecision(decisionId) {
    this._panel.webview.postMessage({ command: "focusOnDecision", decisionId });
  }
  dispose() {
...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode3.languages.registerCodeLensProvider("*", codelensProvider)
  );
  context.subscriptions.push(
    vscode3.commands.registerCommand("decision-tapestry.showDashboard", () => {
      WebViewPanel.createOrShow(context.extensionUri);
    })
  );
  context.subscriptions.push(
    vscode3.commands.registerCommand("decision-tapestry.showDecision", async (decisions) => {
      WebViewPanel.createOrShow(context.extensionUri);
  ...
  context.subscriptions.push(
    vscode3.commands.registerCommand("decision-tapestry.createDecision", async () => {
      ...
      if (title) {
        ...
        if (!workspaceFolders) {
          vscode3.window.showErrorMessage("No workspace folder is open. Cannot create decision.");
          return;
        }
        ...
        try {
          ...
          vscode3.window.showErrorMessage(`Failed to create decision: ${error}`);
        }
      }
    })
  );
