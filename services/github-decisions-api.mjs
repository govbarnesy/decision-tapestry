...
export class GitHubDecisionsAPI {
  ...
  issueToDecision(issue) {
    ...
    const tasks = this.extractTasks(sections.tasks || sections["implementation tasks"] || "");
    
    // Get related PRs from the issue
    const relatedPRs = this.extractReferences(sections["implementation prs"] || "");
    const relatedDecisions = this.extractReferences(sections["related decisions"] || "");
    ...
    return {
      ...
      affected_components: this.parseList(sections["affected components"] || ""),
      tasks: tasks,
  ...
  async getProjectDecisions(projectNumber) {
    ...
    try {
      ...
      return result.repository.projectV2.items.nodes
        .filter(item => item.content && item.content.labels.nodes.some(l => l.name === 'decision'))
        .map(item => {
          ...
          item.fieldValues.nodes.forEach(field => {
            if (field.field?.name) {
              decision.project_fields[field.field.name] = field.text || field.name;
            }
          });
