...
class GitHubService {
    ...
    async initialize(token, baseUrl = 'https://api.github.com') {
        try {
            ...
            if (!token) {
                console.warn('⚠️  GitHub token not provided. API rate limited to 60 requests/hour.');
    ...
    async getUser(username) {
        ...
        if (!this.canMakeRequest()) {
            console.warn(`GitHub rate limit exceeded. Reset at: ${this.rateLimit.resetTime}`);
            return null;
        }
    ...
    async getRepository(owner, repo) {
        ...
        try {
            ...
            console.error(`GitHub API error for repo ${owner}/${repo}:`, error.message);
            return null;
        }
    }
