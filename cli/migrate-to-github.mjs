...
class DecisionMigrator {
  ...
  async init() {
    ...
    console.log(
      "This tool will migrate your local decisions.yml to GitHub Issues.\n",
    );
    ...
    const repoUrl = await input({
      ...
      validate: (value) =>
        /^[^/]+\/[^/]+$/.test(value) || "Format must be owner/repo",
    });
    ...
    try {
      ...
      console.error(
        chalk.red("✗ Failed to connect to repository:", error.message),
      );
      process.exit(1);
    }
  }
  ...
  async migrateDecision(decision, dryRun = false) {
    ...
    try {
      ...
      console.error(
        chalk.red(
          `Failed to create issue for decision #${decision.id}:`,
          error.message,
        ),
      );
      return null;
    ...
    for (const decision of decisions) {
      ...
      for (const refDecision of referencingDecisions) {
        ...
        try {
          ...
          console.log(
            chalk.gray(
              `  Would update decision #${refNewId} to reference #${newId}`,
            ),
          );
        } catch {
          console.error(
            chalk.red(`  Failed to update references for #${refNewId}`),
          );
        }
      }
    }
  ...
  async run() {
    ...
    if (mapped > 0) {
      console.log(
        chalk.yellow(`⚠️  Found existing mapping for ${mapped} decisions`),
      );
    ...
    for (const decision of toMigrate) {
      console.log(
        chalk.blue(`Migrating decision #${decision.id}: ${decision.title}`),
      );

      if (this.mapping[decision.id]) {
        console.log(
          chalk.gray(
            `  Already migrated as issue #${this.mapping[decision.id]}`,
          ),
        );
        continue;
      }
    ...
    if (!dryRun && mode !== "dry") {
      ...
      console.log(chalk.bold("\n✅ Migration complete!\n"));
      console.log(
        chalk.green(`Migrated ${Object.keys(this.mapping).length} decisions`),
      );
      ...
      console.log(
        "3. Update your CI/CD to use GitHub Issues instead of decisions.yml",
      );
