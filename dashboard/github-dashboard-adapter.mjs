...
import {
  projectQueries,
  buildDecisionSearchQuery,
} from "../services/github-project-queries.mjs";
...
export class GitHubDashboardAdapter {
    ...
    try {
      ...
      decisions.sort((a, b) => {
        switch (options.sortBy) {
          case "updated":
            return (
              new Date(b.date.last_updated) - new Date(a.date.last_updated)
            );
          case "created":
            return (
              new Date(b.date.decision_date) - new Date(a.date.decision_date)
            );
          case "id":
            return b.id - a.id;
          default:
  ...
  async searchDecisions(query) {
    ...
    return result.search.nodes
      .filter(
        (issue) =>
          issue.repository.owner.login === this.owner &&
  ...
  async getDecisionActivity(decisionId) {
    const result = await this.graphqlClient(
      projectQueries.getDecisionActivity,
      {
        owner: this.owner,
        repo: this.repo,
  ...
  async getRepositoryStats() {
    const result = await this.graphqlClient(
      projectQueries.getRepositoryInsights,
      {
        owner: this.owner,
        repo: this.repo,
      },
  ...
  async getProjects() {
    const result = await this.graphqlClient(
      projectQueries.listRepositoryProjects,
      {
        owner: this.owner,
        repo: this.repo,
      },
