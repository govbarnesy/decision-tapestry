...
class CommitTimelineViz extends LitElement {
    ...
    _getTimelineData() {
        ...
        const groups = groupByTimePeriod(decisionsWithDate, this.groupBy, this.dateType);
    ...
    _handlePeriodClick(period, decisions) {
        ...
        this.dispatchEvent(new CustomEvent('timeline-item-click', {
    ...
    _renderTimeline() {
        ...
        return html`
            <div class="timeline-container">
                <div class="timeline-track">
                    ${sortedPeriods.map(period => {
                        ...
                        const barHeight = this._calculateBarHeight(decisions.length, maxCount);
                        
                        return html`
                            <div 
                                class="timeline-period ${this.selectedPeriod === period ? 'selected' : ''}"
                                ...
                                >
                                    ...
                                    ${commitCount > 0 ? html`
                                        <div class="commit-dots">
                                            ${Array(Math.min(commitCount, 10)).fill(0).map(() => html`
                                                <div class="commit-dot"></div>
                                            `)}
                                            ${commitCount > 10 ? html`<span style="font-size: 10px; color: var(--text-secondary);">+${commitCount - 10}</span>` : ''}
                                        </div>
                                    ` : ''}
                                    <div class="tooltip">
                                        ${decisions.length} decision${decisions.length !== 1 ? 's' : ''}
                                        ${commitCount > 0 ? html`<br>${commitCount} commit${commitCount !== 1 ? 's' : ''}` : ''}
                                    </div>
                                </div>
                                <span class="timeline-label">${this._formatPeriodLabel(period)}</span>
                            </div>
                        `;
                    })}
                </div>
            ...
            <div class="timeline-legend">
                ...
                <div class="legend-item">
                    <div class="legend-dot" style="width: 6px; height: 6px; opacity: 0.6;"></div>
            ...
            <div class="timeline-summary">
                <div class="summary-stats">
                    ...
                    <div class="summary-stat">
                        <div class="summary-value">${Object.values(groups).reduce((sum, decisions) => sum + decisions.length, 0)}</div>
                    ...
                    <div class="summary-stat">
                        <div class="summary-value">${(totalCommits / Object.values(groups).reduce((sum, decisions) => sum + decisions.length, 0)).toFixed(1)}</div>
        ...
        return html`
            <div class="timeline-header">
                ...
                <div class="timeline-controls">
                    ${['day', 'week', 'month', 'quarter', 'year'].map(period => html`
                        <button 
                            class="control-button ${this.groupBy === period ? 'active' : ''}"
