...
class FileStatusDisplay extends LitElement {
  ...
  _renderFileSection(status, files) {
    ...
    return html`
      <div class="status-group ${isCollapsed ? "collapsed" : ""}">
        <div
          class="status-header"
          @click="${() => this._toggleSection(status)}"
        >
        ...
        <ul class="file-list">
          ${files.map((file) => html` <li class="file-item">${file}</li> `)}
        </ul>
      </div>
    `;
  }
  ...
  render() {
    if (!this.fileStatus) {
      return html`
        <div class="empty-message">No file status information available.</div>
      `;
    }

    const syncStatus = this._checkSyncStatus();
    const hasAnyFiles =
      (this.fileStatus.created?.length || 0) +
        (this.fileStatus.modified?.length || 0) +
        (this.fileStatus.deleted?.length || 0) +
        (this.fileStatus.missing?.length || 0) >
      0;
    ...
    return html`
      <div class="components-header">
        ...
        ${syncStatus !== null
          ? html`
              <span
                class="sync-badge ${syncStatus ? "in-sync" : "out-of-sync"}"
              >
