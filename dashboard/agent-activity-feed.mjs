...
class AgentActivityFeed extends LitElement {
        ...
        return html`
            <div class="feed-header">
                ...
                <div style="display: flex; gap: 0.5rem; align-items: center;">
                    ...
                    <button class="clear-button" @click="${this.clearActivities}">Clear</button>
                </div>
            </div>
    ...
    renderActivity(activity) {
        ...
        const isCompleted = activity.type === 'task_completion' || activity.message?.includes('completed');
        
        return html`
            <div class="activity-item ${isRecent ? 'recent' : ''} ${isError ? 'error' : ''} ${isCompleted ? 'completed' : ''}"
                ...
                <div class="activity-header">
                    ...
                    <span class="activity-timestamp">${this.formatTimestamp(activity.timestamp)}</span>
                </div>
                ...
                <div class="activity-details">
                    ...
                    ${activity.currentTask ? html`
                        <span class="task-info">Task: ${activity.currentTask}</span>
                    ` : ''}
                    
                    ${activity.completedTasks !== undefined ? html`
                        <span class="task-info">Progress: ${activity.completedTasks}/${activity.totalTasks || '?'}</span>
                    ` : ''}
                </div>
            </div>
        `;
    ...
    hasNewActivities() {
        ...
        const recentActivities = this.activities.filter(activity => 
            new Date(activity.timestamp) > this.lastActivityTime
        );
    ...
    getStatus() {
        return {
            ...
            recentActivities: this.activities.filter(a => this.isRecentActivity(a)).length,
