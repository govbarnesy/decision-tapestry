...
class AuthorFilter extends LitElement {
  ...
  _isSelected(author) {
    ...
    return this.selectedAuthors.some(
      (selected) => this._getAuthorKey(selected) === key,
    );
  }
  ...
  _toggleAuthor(author) {
    ...
    if (isCurrentlySelected) {
      newSelection = this.selectedAuthors.filter(
        (selected) => this._getAuthorKey(selected) !== key,
      );
    } else {
  ...
  _dispatchFilterChange() {
    this.dispatchEvent(
      new CustomEvent("author-filter-change", {
  ...
  _filterAuthors(authors) {
    ...
    return authors.filter((author) => {
      ...
      return (
        displayName.includes(this._searchTerm) ||
  ...
  _renderAuthorItem(author, decisionCount) {
    ...
    return html`
      ...
      >
        <github-avatar .author="${author}" size="medium"></github-avatar>
        <div class="author-details">
          <div class="author-name" title="${displayName}">${displayName}</div>
          ${isGitHubUser && author.github_username
            ? html`
        ...
        <div class="author-grid">
          ${filteredAuthors.map((author) =>
            this._renderAuthorItem(
              author,
              decisionCounts.get(this._getAuthorKey(author)) || 0,
            ),
          )}
        </div>
