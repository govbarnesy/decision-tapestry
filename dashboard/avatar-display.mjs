...
class GitHubAvatar extends LitElement {
  ...
  _renderAvatar() {
    ...
    return html` <div class="fallback-avatar ${sizeClass}">${initials}</div> `;
  }
  ...
  render() {
    ...
    const avatarContent = html`
      <div class="avatar-wrapper">${this._renderAvatar()}</div>
      ${this.showName
        ? html`<span class="author-name">${displayName}</span>`
        : ""}
    `;
