...
class DecisionMap extends LitElement {
  ...
  set nodes(newVal) {
    ...
    if (this._nodesDataSet) {
      ...
      const toRemove = this._nodesDataSet
        .getIds()
  ...
  set edges(newVal) {
    ...
    if (this._edgesDataSet) {
      ...
      const toRemove = this._edgesDataSet
        .getIds()
        .filter((id) => !newIds.has(id));
  ...
  focusOnNode(nodeId) {
    ...
    this._debounce(() => {
      if (this._network) {
        ...
        if (
          nodePosition &&
          (viewPosition.x !== nodePosition.x ||
            viewPosition.y !== nodePosition.y)
        ) {
  ...
  _clusterByCategory(category) {
    const categoryNodes = this._nodes.filter(
      (node) => this._inferCategory(node) === category,
    );
    ...
    const clusterOptions = {
      ...
      clusterNodeProperties: {
        ...
        color: {
          ...
          highlight: {
            background: this._lightenColor(
              this._categoryColors[category] || "#95A5A6",
              0.3,
            ),
            border: this._categoryColors[category] || "#95A5A6",
          },
  ...
  _maybeInitNetwork() {
    const container =
      this.shadowRoot && this.shadowRoot.getElementById("network");
