  ...
  render() {
    ...
    return html`
      ${this.backlogItems.map((item) => {
        ...
        const rationaleHtml =
          item.rationale && item.rationale.length
            ? html`<div>
                <strong>Rationale:</strong>
                <ul>
                  ${item.rationale.map((r) => html`<li>${r}</li>`)}
                </ul>
              </div>`
            : "";
        // Tradeoffs as bullet list
        const tradeoffsHtml =
          item.tradeoffs && item.tradeoffs.length
            ? html`<div>
                <strong>Tradeoffs:</strong>
                <ul>
                  ${item.tradeoffs.map((t) => html`<li>${t}</li>`)}
                </ul>
              </div>`
            : "";
        // Tasks as checklist
        ...
        return html`
          <div class="backlog-item" data-backlog-id=${item.id}>
            ...
            ${rationaleHtml} ${tradeoffsHtml} ${tasksHtml} ${notesHtml}
          </div>
        `;
      })}
    `;
