    ...
    this.decisions.forEach((decision) => {
      ...
      if (chain.length > 1) {
        const pathway = {
          ...
          score:
            chain.length * 10 +
            chain.filter((d) => d.status === "Done").length * 5,
      ...
      if (decisions.length > 0) {
        const relatedDecisions = decisions.concat(
          decisions.flatMap((d) =>
            d.related_to
              ? d.related_to
                  .map((id) => this.decisions.find((dec) => dec.id === id))
                  .filter(Boolean)
              : [],
          ),
        );
        ...
        // Remove duplicates
        const unique = relatedDecisions.filter(
          (d, i, arr) => arr.findIndex((item) => item.id === d.id) === i,
        );
    ...
    return html`
      ...
      >
        ...
        <div class="pathway-chain">
          ${pathway.decisions.map(
            (decision, index) => html`
              ${index > 0 ? html`<span class="pathway-arrow">→</span>` : ""}
              <span
                class="pathway-decision ${decision.status === "Superseded"
                  ? "superseded"
                  : ""}"
