...
class TimeRangePicker extends LitElement {
    ...
    this.decisions.forEach((decision) => {
      ...
      if (date) {
        const bucketIndex = Math.floor(
          (date.getTime() - startTime) / bucketSize,
        );
  ...
  _applyPreset(preset) {
    ...
    switch (preset) {
      case "all": {
        const stats = calculateTimeStatistics(this.decisions);
        this.startDate = stats.dateRange.earliest
          ? stats.dateRange.earliest.toISOString().split("T")[0]
          : "";
        this.endDate = stats.dateRange.latest
          ? stats.dateRange.latest.toISOString().split("T")[0]
          : "";
        break;
      }
      case "today":
  ...
  _renderHistogram() {
    ...
    if (bars.length === 0) {
      return html`<div
        style="text-align: center; color: var(--text-secondary); font-size: 12px; padding: 20px;"
      >
        No data to display
      </div>`;
    }
          ...
          (bar) => html`
            ...
            >
              <div class="histogram-tooltip">
                ...
                ${bar.start.toLocaleDateString("en-US", {
                  month: "short",
                  year: "numeric",
                })}
              </div>
    ...
    return html`
      ...
      <div class="picker-body">
          ...
          ? html`
              <div class="presets-section">
                ...
                <div class="presets-grid">
                  <button
                    class="preset-button"
                    @click=${() => this._applyPreset("all")}
                  >
                    All Time
                  </button>
                  <button
                    class="preset-button"
                    @click=${() => this._applyPreset("today")}
                  >
                    Today
                  </button>
                  <button
                    class="preset-button"
                    @click=${() => this._applyPreset("week")}
                  >
                    Last 7 Days
                  </button>
                  <button
                    class="preset-button"
                    @click=${() => this._applyPreset("month")}
                  >
                    Last Month
                  </button>
                  <button
                    class="preset-button"
                    @click=${() => this._applyPreset("quarter")}
                  >
                    Last Quarter
                  </button>
                  <button
                    class="preset-button"
                    @click=${() => this._applyPreset("year")}
                  >
                    Last Year
                  </button>
                  <button
                    class="preset-button"
                    @click=${() => this._applyPreset("ytd")}
                  >
                    Year to Date
                  </button>
                </div>
        ...
        <div class="range-summary">
          <strong>${decisionsInRange}</strong> of
          <strong>${totalDecisions}</strong> decisions in range:
...
// Timestamp: 2025-07-16T05:35:55.776Z
...
// Timestamp: 2025-07-16T05:35:56.013Z
